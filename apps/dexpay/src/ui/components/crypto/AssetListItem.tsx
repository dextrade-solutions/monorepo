...
export function AssetListItem({
  ...
  return (
    <Card sx={{ borderRadius: 2, boxShadow: 3 }}> {/* Improved card styling */}
      <CardContent
        sx={{
          display: 'flex',
          alignItems: 'center',
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <UrlIcon url={getCoinIconByUid(asset.uid)} />
          <Typography variant="subtitle1" fontWeight="medium">
            {asset.symbol}
          </Typography>
        </Box>
        <Box sx={{ textAlign: 'right' }}> {/* Added a right-aligned box for balance */}
