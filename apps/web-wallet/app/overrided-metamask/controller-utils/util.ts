...
async function parseRepsonse(response: Response) {
  if (!response.body) {
    return null;
  }
  ...
  if (!response.ok) {
    ...
    const msg =
      data?.message ||
      data?.error ||
      `Fetch failed with status '${response.status}' for request '${response.url}'`;
...
export async function successfulFetch(
  ...
  const data = await parseRepsonse(response);
...
export async function handleFetch(request: string, options?: RequestInit) {
  return successfulFetch(request, options);
}
...
export function query(
  ...
  return new Promise((resolve, reject) => {
    ...
    if (
      ...
      ethQuery.sendAsync(
        { method, jsonrpc: '2.0', id: createRandomId(), params: args },
        cb,
      );
    }
  });
