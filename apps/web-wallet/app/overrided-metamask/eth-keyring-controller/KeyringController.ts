...
class SessionKeyring extends HDKeyring {
  constructor(...args: any[]) {
    super(...args);
    const seed = randomBytes(32);
    this.session = HDKey.fromMasterSeed(seed);
  }
}

const defaultKeyringBuilders = [
  keyringBuilderFactory(SimpleKeyring),
  keyringBuilderFactory(SessionKeyring),
];
...
class KeyringController extends EventEmitter {
   ...
   *
   * Only supports 'Simple Key Pair'. (In dextrade support for both types)
   *
...
async function displayForKeyring(
  ...
  let session;
  if (keyring.session) {
    session = Buffer.from(keyring.session.publicKey).toString('hex');
  }
