...
export class Engine extends ISingleEthereumEngine {
  ...
  public approveSession: ISingleEthereumEngine['approveSession'] = async (
    ...
    const { id, chainIds, accounts } = params;
    ...
    const requiredParsed =
      requiredChain && parseInt(parseChain(requiredChain), 10);
    const approvedChains = [...chainIds];

    if (requiredParsed && !approvedChains.includes(requiredParsed)) {
      approvedChains.push(requiredParsed);
    }
    ...
    const approveParams = {
      ...
      namespaces: {
        [EVM_IDENTIFIER]: {
          ...
          accounts: approvedChains
            .map((chain) => formatAccounts(accounts, chain))
            .flat(),
          chains: approvedChains.map((chain) =>
            prefixChainWithNamespace(chain),
          ),
    ...
    if (optionalMethods) {
      approveParams.namespaces[EVM_IDENTIFIER].methods =
        approveParams.namespaces[EVM_IDENTIFIER].methods
  ...
  public approveRequest: ISingleEthereumEngine['approveRequest'] = async (
    ...
    if (this.shouldHandlePendingInternalRequest(id, true)) {
      return;
    }
  ...
  public rejectRequest: ISingleEthereumEngine['rejectRequest'] = async (
    ...
    if (this.shouldHandlePendingInternalRequest(id, false)) {
      return;
    }
    await this.web3wallet.respondSessionRequest({
      topic,
  ...
  public approveAuthRequest: ISingleEthereumEngine['approveAuthRequest'] =
    async (params) => {
  ...
  public rejectAuthRequest: ISingleEthereumEngine['rejectAuthRequest'] = async (
    params,
  ) => {
  ...
  public formatAuthMessage: ISingleEthereumEngine['formatAuthMessage'] = (
    payload,
    address,
  ) => {
  ...
  private onSessionRequest = async (
    event: SingleEthereumTypes.SessionRequest,
  ) => {
    event.params.chainId = parseChain(event.params.chainId);

    if (
      parseInt(event.params.chainId, 10) !== this.chainId ||
      this.isSwitchChainRequest(event)
    ) {
  ...
  private switchEthereumChain = async (
    ...
    if (this.isSwitchChainRequest(event)) {
      return;
    }
  ...
  private shouldHandlePendingInternalRequest = (
    id: number,
    isSuccess: boolean,
  ) => {
    const internalRequest = this.pendingInternalRequests.find(
      (r) => r.id === id,
    );
    if (internalRequest) {
      this.pendingInternalRequests = this.pendingInternalRequests.filter(
        (r) => r.id !== id,
      );
      isSuccess ? internalRequest.resolve() : internalRequest.reject();
    }
    return Boolean(internalRequest);
  };

  private isSwitchChainRequest(event: SingleEthereumTypes.SessionRequest) {
    try {
      const chainId = parseInt(
        (event.params.request?.params as any[])?.[0].chainId,
        10,
      );
      return (
