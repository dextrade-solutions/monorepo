...
export const parseProposal = (proposal: ProposalTypes.Struct) => {
  ...
  const optionalChains =
    proposal.optionalNamespaces?.[EVM_IDENTIFIER]?.chains || [];
  ...
  if (parsedProposal.optionalNamespaces?.[EVM_IDENTIFIER]) {
    parsedProposal.optionalNamespaces[EVM_IDENTIFIER].chains =
      parseChains(optionalChains);
  }
