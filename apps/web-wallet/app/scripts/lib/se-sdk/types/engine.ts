...
export abstract class ISingleEthereumEngine {
  ...
  public abstract getActiveSessions():
    | Record<string, SessionTypes.Struct>
    | undefined;

  // query all pending session requests (SIGN)
  public abstract getPendingSessionProposals():
    | Record<number, ProposalTypes.Struct>
    | undefined;

  // query all pending session requests (SIGN)
  public abstract getPendingSessionRequests():
  ...
  public abstract rejectAuthRequest(params: {
    id: number;
    error: ErrorResponse;
  }): Promise<void>;

  public abstract getPendingAuthRequests(): Record<
    number,
    SingleEthereumTypes.PendingAuthRequest
  >;
}
