...
import { swapDEXProviders, swapOTCProviders } from './constants';
// import { quote } from './dex/dex';
// import { getQuote } from './dex/getQuote';
// import { executeRoute } from './dex/routing/routing';
import SwapsExchanger from './swaps-exchanger';
...
const initialState = {
  swapsState: {
    ...
    swapsFeatureFlags: {},
    p2pApiKeyClient: null,
    p2pApiKeyExchanger: null,
    p2pCurrentExchange: null,
    p2pExchangesLoading: true,
    p2pExchangesList: [],
    swapOTCProviders,
    otcCurrentExchange: null,
    swapDEXProviders,
    dexCurrentExchange: null,
  },
};
...
export default class SwapsController {
  constructor({
    ...
    this.getTxController = getTxController;

    this.swapsExchanger = new SwapsExchanger(this);
  }
  ...
  async p2pExchangesFilter(params, loadMore) {
    this.setP2PExchangesLoading(true);
    if (!loadMore) {
      this.setP2PExchanges([]);
    }
    const items = await this.dextradeController.api.filterExchanges(params);
    this.setP2PExchangesLoading(false);
    if (loadMore) {
      this.setP2PExchanges([
        ...this.store.getState().swapsState.p2pExchangesList,
        ...items,
      ]);
    } else {
      this.setP2PExchanges(items);
    }
  }

  async clientP2PExchangeStart(type, params) {
    const currentExchange =
      await this.dextradeController.api.clientExchangeStart(type, params);
    this.setP2PCurrentExchange(currentExchange);
  ...
  setP2PCurrentExchange(exchange) {
    const { swapsState } = this.store.getState();
    this.store.updateState({
      swapsState: { ...swapsState, p2pCurrentExchange: exchange },
    });
  }

  setP2PExchanges(items) {
    const { swapsState } = this.store.getState();
    this.store.updateState({
      swapsState: { ...swapsState, p2pExchangesList: items },
    });
  }

  setP2PExchangesLoading(loading) {
    const { swapsState } = this.store.getState();
    this.store.updateState({
      swapsState: { ...swapsState, p2pExchangesLoading: loading },
    });
  }
