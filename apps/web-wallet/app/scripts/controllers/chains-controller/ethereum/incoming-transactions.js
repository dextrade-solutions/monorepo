...
const generateRequest = (action, address, fromBlock, chainId) => {
...
export default class IncomingTransactionsController {
  ...
  async _update(address, newBlockNumberDec) {
    ...
    try {
      ...
      const mostRecentlyFetchedBlock = currentState.incomingTxLastFetchedBlock;
      ...
      this.chainControllerStore.updateState({
        incomingTxLastFetchedBlock: newMostRecentlyFetchedBlock + 1,
  ...
  async _getNewIncomingTransactions(address, fromBlock, chainId) {
    const txListRequest = generateRequest(
      'txlist',
      address,
      fromBlock,
      chainId,
    );
    const tokentxRequest = generateRequest(
      'tokentx',
      address,
      fromBlock,
      chainId,
    );
    const responses = await Promise.all([txListRequest(), tokentxRequest()]);
    const result = flatten(responses);
    let newIncomingTxs = [];
    if (result.length > 0) {
      const remoteTxList = {};
      const remoteTxs = [];
  ...
  _normalizeTxFromEtherscan(etherscanTransaction, chainId) {
    ...
    const status =
      (!etherscanTransaction.isError && TransactionStatus.confirmed) ||
    ...
    let symbol =
      ...
      this.sharedProvider.nativeToken.symbol;
    ...
    const txParams = {
      ...
      localId: getTokenIdLocal({
        ...
        contract: etherscanTransaction.contractAddress,
      }),
    };
