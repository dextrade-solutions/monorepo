...
export const incomingTxListSelector = (state) => {
  ...
  return Object.values(state.metamask.usedNetworks).reduce(
    (acc, { incomingTransactions = {} }) => {
      return [...acc, ...Object.values(incomingTransactions)];
    },
    [],
  );
};
...
export const selectedAddressTxListSelector = createSelector(
  ...
  (transactions = [], smTransactions = []) => {
    return transactions.concat(smTransactions);
  },
);
...
export const transactionsSelector = createSelector(
  ...
  (selectedAddress, subSelectorTxList = [], selectedAddressTxList = []) => {
    const txsToRender = selectedAddressTxList.concat(subSelectorTxList);

    return txsToRender
      .filter(({ walletAddress }) => walletAddress === selectedAddress)
...
export const nonceSortedTransactionsSelector = createSelector(
  ...
  (transactions = []) => {
    ...
    transactions.forEach((transaction) => {
      ...
      if (true || type === TransactionType.incoming) {
...
export const p2pTransactionsSelector = createSelector(
  nonceSortedTransactionsSelector,
  (transactions = []) =>
    (filters = {}) => {
      let filteredList = transactions.filter(({ primaryTransaction }) =>
        filters.types.includes(primaryTransaction.exchangerType),
      );
      ...
      if (filters.unpaid) {
        filteredList = filteredList.filter(
          ({ primaryTransaction }) =>
            primaryTransaction.status === TransactionStatus.unapproved &&
            [
              ExchangePairType.cryptoFiat,
              ExchangePairType.fiatCrypto,
              ExchangePairType.fiatFiat,
            ].includes(primaryTransaction.pairType),
        );
      }
      if (filters.exchangerSettingsId) {
        filteredList = filteredList.filter(
          ({ primaryTransaction }) =>
            primaryTransaction.exchangerSettings.exchangerSettingsId ===
            filters.exchangerSettingsId,
        );
      }
      return filteredList.sort((a, b) => b.primaryTransaction.time - a.primaryTransaction.time);
    },
);
