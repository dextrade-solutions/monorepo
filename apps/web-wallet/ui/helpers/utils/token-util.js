...
export function getTokenMetadata(tokenAddress, tokenList) {
  return tokenAddress && tokenList[tokenAddress];
}
...
export function getTokenFiatAmount(
  ...
  // shown to the user
  if (conversionRate <= 0 || tokenAmount === undefined) {
    return undefined;
  }

  let currentTokenInFiat = new Numeric(tokenAmount, 10);

  if (tokenSymbol !== currentCurrency.toUpperCase()) {
    currentTokenInFiat = currentTokenInFiat.applyConversionRate(conversionRate);
  }
