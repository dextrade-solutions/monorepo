...
import { ExchangerType } from './exchanger';
...
export enum TransactionType {
  ...
  atomicSwapClaim = 'claimSwap',
  atomicSwapRefund = 'refundSwap',
}
...
export interface AtomicSafe {
  id: string;
  data?: {
    hashLock: string;
    claimed: boolean;
    refunded: boolean;
    // offset from block time or block height
    expiration: number;
    amount: string;
    sender: string;
    recepient: string;
...
export interface TransactionMeta {
  /* Atomic Swap */
  toSafe?: AtomicSafe; // claimed or not
  fromSafe?: AtomicSafe;
  hashLock?: string;
  ...
  exchangerSettings: {
    ...
    priceAdjustmentInCoin1?: number;
  };

  // TODO: Move it in exchanger settings
  // otc?: {
  //   id: string;
  //   provider: string;
  // };
  ...
  swapMetaData?: {
    ...
    token_to_amount: number;
    // eslint-disable-next-line camelcase
    token_to: string;
    // eslint-disable-next-line camelcase
    token_from_amount: number;
    // eslint-disable-next-line camelcase
    token_from: string;
  };
  /* End Swap parameters */
