...
import abiMulticall from './abiMulticall.json';
...
const uniswapInterface = new Interface(abiMulticall);
...
export function isEIP1559Transaction(transaction) {
  return (
    ...
    Number(hexToDecimal(transaction?.txParams?.maxFeePerGas)) > 0
  );
}
...
export function parseStandardTokenTransactionData(data) {
  ...
  try {
    return uniswapInterface.parseTransaction({ data });
  } catch {
    // ignore and return undefined
  }
